...
class FaceRecognition(BaseRecognition):
    ...
    def encode_faces(self, facesBbox, image):
        encodings = []
        names = []
        try:
        ...
        for person in train_dir:
            if person not in self.know_faces[0]:   
                ...
                for person_img in pix:
                    M_face = None
                    M_area = -float('inf')
                    for top, right, bottom, left in facesBbox:
                    ...
                    if M_face is not None:
                        face_enc = face_recognition.face_encodings(image, known_face_locations=[M_face])[0]
