...
class FaceRecognition(BaseRecognition):
    ...
    def callback(self, *args):
        ...
        face_recognition = Recognitions2D()
        img = None
        points = None
        if len(args):
            img = args[0]
        ...
        with torch.no_grad():
            ...
            face_recognition.header = h
            face_recognition.image_rgb = copy(img)
            face_recognition.points = copy(points)

            people_recognition = copy(face_recognition)
            ...
            if len(face_recognition.descriptions) > 0:
                self.object_recognition_publisher.publish(face_recognition)
